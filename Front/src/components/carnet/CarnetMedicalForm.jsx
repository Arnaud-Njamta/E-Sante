...
export default function CarnetMedicalForm({
  form,
  setForm,
  textes,
  onSave,
  ...
  return (
    <>
      ...
      <Card style={{ padding: 24 }}>
        ...
        />
        <TagInput
          label="Pathologies / maladies chroniques"
          values={form.pathologies}
          onChange={(v) => setForm({ ...form, pathologies: v })}
          placeholder="Ex. Diabète, Hypertension…"
        />
        <TagInput
          label="Antécédents familiaux"
          values={form.antecedents_familiaux}
          onChange={(v) => setForm({ ...form, antecedents_familiaux: v })}
        />
        <TagInput
          label="Antécédents chirurgicaux"
          values={form.antecedents_chirurgicaux}
          onChange={(v) => setForm({ ...form, antecedents_chirurgicaux: v })}
        />
        <TagInput
          label="Traitements habituels"
          values={form.traitements_habituelles}
          onChange={(v) => setForm({ ...form, traitements_habituelles: v })}
        />
        <TagInput
          label="Vaccinations"
          values={form.vaccinations}
        ...
        {isFirstSetup && (
          <>
            <label style={{ display: 'flex', gap: 10, marginTop: 20, fontSize: '0.85rem', cursor: 'pointer' }}>
              <input
                type="checkbox"
                checked={form.activer_carnet}
                onChange={(e) => setForm({ ...form, activer_carnet: e.target.checked })}
              />
            ...
            {form.activer_carnet && (
              <label style={{ display: 'flex', gap: 10, marginTop: 10, fontSize: '0.85rem', cursor: 'pointer' }}>
                <input
                  ...
                  onChange={(e) => setForm({ ...form, consentement_carnet: e.target.checked })}
                />
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 10, marginTop: 24 }}>
          <Button onClick={onSave} disabled={saving}>
            <Save size={16} /> {isFirstSetup ? 'Créer mon carnet' : 'Enregistrer les modifications'}
          </Button>
